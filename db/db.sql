...
INSERT INTO `lecture` (`title`, `author`, `description`, `chunks`, `id_category`) VALUES
    ('Ion', 'Liviu Rebreanu', 'Romanul "Ion" este o operă fundamentală a literaturii române, urmărind povestea lui Ion, un tânăr care își dorește pământ mai mult decât orice. Acesta este dispus să renunțe la iubirea adevărată pentru a obține pământul dorit, ceea ce pune în lumină legătura strânsă dintre om și pământ în contextul vieții rurale românești.', 112, (SELECT `id` FROM `lecture_category` WHERE `name` = 'Epic')),
    ('Ultima noapte de dragoste, întâia noapte de război', 'Camil Petrescu', 'Opera se remarcă prin introspecția psihologică, prezentând dilemele și conflictele interioare ale personajului principal, Ștefan Gheorghidiu. Romanul explorează temele trădării, geloziei și realităților brutale ale războiului, în contextul Primului Război Mondial.', 68, (SELECT `id` FROM `lecture_category` WHERE `name` = 'Epic')),
    ...
    ('Iona', 'Marin Sorescu', 'Dramă modernă ce reinterpretează povestea biblică a lui Iona, transformând-o într-o meditație despre condiția umană, izolare și căutarea sensului vieții.', 6, (SELECT `id` FROM `lecture_category` WHERE `name` = 'Dramă')),
    ('Cel mai iubit dintre pământeni', 'Marin Preda', 'Roman postbelic profund, explorând tema libertății individuale în fața opresiunii și a alienării sociale prin povestea tragică a profesorului Victor Petrini.', 101, (SELECT `id` FROM `lecture_category` WHERE `name` = 'Epic')),
    ('Maitreyi', 'Mircea Eliade', 'Bazat pe propria experiență a autorului în India, romanul este o poveste de dragoste pasională, explorând contrastul între culturi și consecințele unui astfel de șoc cultural.', 41, (SELECT `id` FROM `lecture_category` WHERE `name` = 'Epic')),
    ('Patul lui Procust', 'Camil Petrescu', 'Un roman filozofic ce abordează ideea relativității adevărului și a percepției, prezentând conflictele interioare și exterioare ale protagonistului Fred Vasilescu.', 85, (SELECT `id` FROM `lecture_category` WHERE `name` = 'Epic')),
    ...
    -- Liric
INSERT INTO `lecture` (`title`, `author`, `description`, `chunks`, `id_category`) VALUES
    ('Luceafărul', 'Mihai Eminescu', 'Considerată capodopera lui Eminescu, "Luceafărul" reprezintă lupta dintre ideal și realitate. Prin relația dintre Hyperion și Cătălina, poezia discută dualitatea dintre spirit și materie, aspirație și destin.', 4, (SELECT `id` FROM `lecture_category` WHERE `name` = 'Liric')),
    ('Scrisoarea I', 'Mihai Eminescu', 'Reflectând asupra decadenței Romei antice, aceasta poezie explorează efemeritatea imperiilor și trăinicia timpului.', 1, (SELECT `id` FROM `lecture_category` WHERE `name` = 'Liric')),
    ('Scrisoarea III', 'Mihai Eminescu', 'Un omagiu adus lui Ștefan cel Mare, poezia combină patriotismul cu reflecții istorice și filozofice despre destinul național.', 2, (SELECT `id` FROM `lecture_category` WHERE `name` = 'Liric')),
    ...
    ('Floare albastră', 'Mihai Eminescu', 'Evocând iubirea neîmplinită și aspirația spre ideal, poezia se folosește de simbolul floarei albastre pentru a exprima dorința arzătoare.', 1, (SELECT `id` FROM `lecture_category` WHERE `name` = 'Liric')),
    ...
    ('Testament', 'Tudor Arghezi', 'O reflexie profundă asupra moștenirii spirituale, Arghezi consideră creația sa ca un dar pentru posteritate.', 1, (SELECT `id` FROM `lecture_category` WHERE `name` = 'Liric')),
    ('Flori de mucigai', 'Tudor Arghezi', 'Explorând ideea de frumusețe în decădere, poezia juxtapune imagini ale vieții și morții, subliniind ciclicitatea existenței.', 1, (SELECT `id` FROM `lecture_category` WHERE `name` = 'Liric')),
    ('Eu nu strivesc corola de minuni a lumii', 'Lucian Blaga', 'Poezia discută relația dintre om și misterul universului, subliniind respectul față de secretele existenței.', 1, (SELECT `id` FROM `lecture_category` WHERE `name` = 'Liric')),
    ('În grădina Ghetsemani', 'Lucian Blaga', 'Evocând grădina biblică, poezia reflectă asupra sacrificiului și trăirilor intense în fața destinului.', 1, (SELECT `id` FROM `lecture_category` WHERE `name` = 'Liric')),
    ('La tiganci', 'Lucian Blaga', 'Poezia ilustrează o lume de vis și mister, cu elemente suprarealiste și reflecții asupra realității și fanteziei.', 26, (SELECT `id` FROM `lecture_category` WHERE `name` = 'Liric')),
    ('Leoaică tânără, iubirea...', 'Nichita Stănescu', 'O celebrare a iubirii tinere și pasionale, poezia juxtapune imagini ale vitalității cu cele ale vulnerabilității.', 1, (SELECT `id` FROM `lecture_category` WHERE `name` = 'Liric')),
    ('Rugăciune', 'Octavian Goga', '"Rugăciune" de Octavian Goga este o poezie profundă în care poetul exprimă, în termeni emoționanți, aspirațiile naționale și suferința poporului său. Poezia combină fervoarea religioasă cu pasiunea patriotică, fiind un strigăt către divinitate pentru îndurare și ajutor în vremuri de cumpănă, oglindind astfel angajamentul lui Goga față de valorile naționale și dragostea sa față de țărănimea română.', 1, (SELECT `id` FROM `lecture_category` WHERE `name` = 'Liric')),
    ('Moartea lui Fulger', 'George Coșbuc', 'Poezia prezintă o alegorie a vitejiei și a sfârșitului tragic al eroului mitic Fulger, simbolizând lupta și caducitatea eroismului uman.', 1, (SELECT `id` FROM `lecture_category` WHERE `name` = 'Liric')),
    ('Malul Siretului', 'Vasile Alecsandri', 'Această operă lirică exprimă dragostea lui Alecsandri pentru peisajele românești, în special pentru malurile Siretului, prin descrieri evocative și sentimentale.', 1, (SELECT `id` FROM `lecture_category` WHERE `name` = 'Liric')),
    ('Plumb', 'George Bacovia', 'Poemul "Plumb" este emblematic pentru simbolismul românesc, transmițând stări de melancolie profundă și izolare prin imagini sumbre și dense.', 1, (SELECT `id` FROM `lecture_category` WHERE `name` = 'Liric')),
    ('Lacustră', 'George Bacovia', 'Poezia este expresia simbolistă a stărilor de melancolie și singurătate, evocând atmosfera apăsătoare a peisajelor lacustre sub cerul plumburiu.', 1, (SELECT `id` FROM `lecture_category` WHERE `name` = 'Liric'));

    
    -- Dramă
INSERT INTO `lecture` (`title`, `author`, `description`, `chunks`, `id_category`) VALUES
    ('O scrisoare pierdută', 'I.L. Caragiale', 'Una dintre cele mai reprezentative opere ale lui Caragiale, "O scrisoare pierdută" este o satiră acută la adresa mediului politic românesc de la sfârșitul secolului XIX. Povestea gravitează în jurul unei scrisori compromițătoare pierdute de un politician, care devine obiectul unei goane frenetice. Prin umorul său inconfundabil, Caragiale evidențiază ipocrizia, corupția și oportunistismul care domină societatea. Caracterizarea precisă a personajelor și dialogul savuros transformă piesa într-o capodoperă a teatrului românesc, având o relevanță care depășește epoca sa.', 17, (SELECT `id` FROM `lecture_category` WHERE `name` = 'Dramă')),
    ('Riga Crypto și lapona Enigel', 'Ion Barbu', 'Deși mai cunoscut ca poet, Ion Barbu a scris și această dramă simbolică, "Riga Crypto și lapona Enigel", care explorează tema identității și a contrastului dintre civilizație și natură. Piesa urmărește interacțiunea dintre două personaje centrale, Riga Crypto, un miner, și Lapona Enigel, reprezentând lumea naturală. Dialogul dintre ei aduce în discuție concepte filozofice și conflictele inerente diferențelor lor. Prin această dramă, Barbu oferă o meditație profundă asupra umanității, culturii și relației cu natura.', 1, (SELECT `id` FROM `lecture_category` WHERE `name` = 'Dramă'));
